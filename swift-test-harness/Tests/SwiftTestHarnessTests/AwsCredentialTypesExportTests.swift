...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `AwsCredentialTypes.swiftmodule/`
...
//      `import AwsCredentialTypes` above would fail at compile time.
//
//   2. The static archive `libAwsCredentialTypes.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "AwsCredentialTypes" }` and
//      `flattenPackage = "io.github.kotlinmania.awscredentialtypes"` configuration in
...
//      via the `AwsCredentialTypesLibrary` product.
//
...
final class AwsCredentialTypesExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "AwsCredentialTypes swift module imported cleanly")
    }
}
