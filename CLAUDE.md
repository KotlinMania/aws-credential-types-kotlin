# Claude Code Project Instructions — aws-credential-types-kotlin

## Project Overview

This is **aws-credential-types-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`aws-credential-types`](https://crates.io/crates/aws-credential-types).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/aws-credential-types/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/aws-credential-types>
