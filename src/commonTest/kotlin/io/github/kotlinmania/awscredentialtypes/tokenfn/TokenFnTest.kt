...
class TokenFnTest {
    @Test
    fun provideTokenFnClosureCanBorrow() = runTest {
        ...
        assertEquals("one", one.provideToken().await().getOrThrow().token())
        assertEquals("two", two.provideToken().await().getOrThrow().token())
    }
}
