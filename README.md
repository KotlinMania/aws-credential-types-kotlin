...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`aws-credential-types`](https://crates.io/crates/aws-credential-types).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `aws-credential-types` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

AWS SDK credential types
...
```kotlin
dependencies {
    implementation("io.github.kotlinmania:aws-credential-types-kotlin:0.1.0-SNAPSHOT")
}
```
...
Original work copyrighted by the upstream `aws-credential-types` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
